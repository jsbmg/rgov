...
Recreation.gov. While other recreation.gov "checkers" exist, this one aims to
...
=$ rgov check <campground id(s)> -date <mm-dd-YY> -length <nights>=
...
=$ rgov daemon <campground id(s)> --date <mm-dd-YY> --length <nights> --notifier <notification program> --command <shell command>=
